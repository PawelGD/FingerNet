#!/usr/bin/env python2
#coding=utf-8
import os, sys, cv2, pickle, shutil
...
import argparse
parser = argparse.ArgumentParser(description='Fingerprint processing')
...
def deploy(deploy_set):
    mkdir(output_dir)
    logging.info("Predicting %s:"%(deploy_set))
    ...
    for i in xrange(0,len(img_name)):
        logging.info("%s %d / %d: %s"%(deploy_set, i+1, len(img_name), img_name[i]))
        time_start = time()
        ...
        draw_ori_on_img(image, ori, np.ones_like(seg_out), "%s/%s_ori.png"%(output_dir, img_name[i]))
        draw_minutiae(image, mnt_nms[:,:3], "%s/%s_mnt.png"%(output_dir, img_name[i]))
        misc.imsave("%s/%s_enh.png"%(output_dir, img_name[i]), np.squeeze(enhance_img)*ndimage.zoom(np.round(np.squeeze(seg_out)), [8,8], order=0))
        misc.imsave("%s/%s_seg.png"%(output_dir, img_name[i]), ndimage.zoom(np.round(np.squeeze(seg_out)), [8,8], order=0))
        io.savemat("%s/%s.mat"%(output_dir, img_name[i]), {'orientation':ori, 'orientation_distribution_map':ori_out_1})
        time_afterdraw = time()
...
def main():
    deploy(deploy_set)
    suffixes = {"enh.png": "enhanced", ".mat": "mats", ".mnt": "minutiae_text", "mnt.png": "minutiae_images", "ori.png": "orientation", "seg.png": "segmentation"}
    for f in suffixes.values():
        mkdir(os.path.join(output_dir, f))
    for f in os.listdir(output_dir):
        p = os.path.join(output_dir, f)
        if os.path.isfile(p) and not p.endswith(".py") and not p.endswith(".log"):
            for key in suffixes.keys():
                if p.endswith(key):
                    destination = suffixes[key]
                    break
            shutil.move(p, os.path.join(output_dir, destination, f))
